...
class Maze:
    ...
    def gen_fusion(cls, height: int, width: int) -> 'Maze':
        ...
        for c1, c2 in shuffle_walls:
            if labels[c1] != labels[c2]:
                ...
                count += 1
                labels = {c : labels[c] if labels[c] != labels[c2] else labels[c1] for c in labels}
    ...
    def solve_bfs(self, start:tuple, stop:tuple):
        """ Resous un labyrinthe grace au parcour en largeur

        Argument:
            start (tuple): cellule de depart
            stop (tuple): cellule d'arriver

        Returns:
            dict: chemin des prèdecesseur du point stop au point start 
        """
        #initilisation 
        marks = [start]
        pred = {start:start}
    ...
    def solve_dfs(self, start:tuple, stop:tuple):
        """ Resous un labyrinthe grace au parcour en pronfondeur

        Args:
            start (tuple): _cellule de depart
            stop (tuple): cellule d'arriver

        Returns:
            _dict: chemin des prèdecesseur du point stop au point start 
        """
        #initilisation 
        marks = [start]
        pred = {start:start}
    ...
    def solve_rhr(self, start:tuple, stop:tuple):
        """Resolution d'un labyrinthe avec la main sur le mur 
        ...
        while len(list(set(self.get_cells()) - set(marks))) != 0:
            ...
            if c == stop:
                ...
                tri= []
                base=self.get_reachable_cells(c)
                tri.append((c[0], c[1]-1)) if (c[0], c[1]-1) in base else None
                tri.append((c[0]-1, c[1])) if (c[0]-1, c[1]) in base else None
                tri.append((c[0], c[1]+1)) if (c[0], c[1]+1) in base else None
                tri.append((c[0]+1, c[1])) if (c[0]+1, c[1]) in base else None
                for voisin in tri:
                    if voisin not in marks:
    ...
    def distance_geo(self,c1:tuple,c2:tuple):
        rhr=len(self.solve_rhr(c1, c2))
        dfs=len(self.solve_dfs(c1,c2))
        bfs=len(self.solve_bfs(c1,c2))
        message = f"Deplacement Minimale {dfs}"
        if rhr<dfs and rhr<bfs:
            message=f"Parcour main Droite: {rhr}"
        elif dfs<rhr and dfs<bfs:
            message=f"Parcour en profondeur:{dfs}"
        elif bfs<rhr and bfs<dfs:
            message = f"parcour en largeur {bfs}"
        return message 

    def distance_man(self,c1:tuple,c2:tuple):
       dx= math.sqrt((c1[0] - c2[0])**2) 
       dy=math.sqrt((c1[1] - c2[1])**2)
       return dx + dy
